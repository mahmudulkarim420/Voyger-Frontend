...
import { Search, Bell, ChevronDown, User as UserIcon, LogOut, ShoppingBag, Heart, Menu } from "lucide-react";
...
export const UserNavbar: React.FC<UserNavbarProps> = ({ onToggleMobileSidebar }) => {
  const { user } = useAuthCheck();
  const { signOutNow } = useSignOut("/");
  const pathname = usePathname();
  const [menuOpen, setMenuOpen] = useState(false);

  const userName = user?.name || "Customer Account";
  ...
  return (
    <header className="h-20 bg-white/80 backdrop-blur-md border-b border-gray-200/50 px-6 md:px-8 flex items-center justify-between sticky top-0 z-30 shadow-2xs">
      ...
      <div className="flex items-center gap-4">
        ...
        <Link href="/user" className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-bold text-[#B37068] tracking-widest uppercase mt-0.5 flex items-center gap-1">
              Customer Account
            </span>
          </div>
        </Link>
      ...
      {/* Right Actions: Cart, Wishlist, Notifications & Profile */}
      <div className="flex items-center gap-3">
        ...
        <Link
          href="/cart"
          className="p-2.5 rounded-full bg-gray-100/80 text-gray-600 hover:bg-gray-200/70 hover:text-black transition-colors cursor-pointer relative"
          aria-label="Shopping Cart"
        >
          <ShoppingBag size={18} />
          <span className="absolute top-1 right-1 w-2 h-2 bg-[#B37068] rounded-full" />
        </Link>
          ...
          {menuOpen && (
            <div className="absolute right-0 mt-3 w-52 bg-white border border-gray-200 rounded-2xl shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="px-4 py-2.5 border-b border-gray-100">
                ...
                <p className="text-[11px] text-gray-500 truncate">{user?.email || "customer@voyage.com"}</p>
              </div>
              <Link
                href="/user/profile"
