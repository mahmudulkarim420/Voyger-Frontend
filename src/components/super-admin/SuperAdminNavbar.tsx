...
import { Search, Bell, ChevronDown, User as UserIcon, LogOut, ShieldCheck, Menu } from "lucide-react";
...
const navTabs = [
  ...
  { name: "Categories", href: "/super-admin/categories" },
  { name: "Orders", href: "/super-admin/orders" },
  { name: "Customers", href: "/super-admin/customers" },
  { name: "Analytics", href: "/super-admin/analytics" },
  { name: "Coupons", href: "/super-admin/coupons" },
  { name: "Payments", href: "/super-admin/payments" },
];

export const SuperAdminNavbar: React.FC<SuperAdminNavbarProps> = ({ onToggleMobileSidebar }) => {
  ...
  return (
    <header className="h-20 bg-white/80 backdrop-blur-md border-b border-gray-200/50 px-6 md:px-8 flex items-center justify-between sticky top-0 z-30 shadow-2xs">
      ...
      {/* Right Actions: Search, Notification & Avatar Dropdown */}
          ...
          {menuOpen && (
            <div className="absolute right-0 mt-3 w-52 bg-white border border-gray-200 rounded-2xl shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="px-4 py-2.5 border-b border-gray-100">
                ...
                <span className="inline-flex items-center gap-1 mt-1 text-[9px] font-extrabold bg-[#B37068] text-white px-2 py-0.5 rounded-full">
                  SUPER_ADMIN
                </span>
              </div>
              <Link
